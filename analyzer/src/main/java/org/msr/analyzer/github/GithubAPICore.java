...
public class GithubAPICore {
	...
	public boolean invokeEndpointUntilEnd() {
		return invokeEndpointUntilEnd(false,false);
	}
		...
		do {
			...
			while (!skipRateLimit && nextInvocationAt != -1){
				...
				if (nextInvocationAt != -1) {
					System.out.println("Rate Limit exceeded so sleeping for (seconds): "+ nextInvocationAt );
					try {
			...
			try {
				...
				System.out.println("Error receiving response for "+newURL+" : "+e.getMessage()+ ": "+e.toString());
				retries++;
				System.out.println("Retry "+retries + " Max retries : "+Constants.MAX_GITHUB_API_RETRIES);
	...
	private long rateLimitInvokeAt() {
		GithubRateLimitInvoker ghRateLimit = new GithubRateLimitInvoker(0);
		...
		System.out.println("next invoke at: "+new Date(nextResetTime*(long)1000));
	...
	public boolean invokeEndpointUntilEnd(boolean isNonArrayResponse) {
		return invokeEndpointUntilEnd(isNonArrayResponse,false);
	}
