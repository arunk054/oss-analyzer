...
public class TrendBuilderOld {
	...
	public TrendBuilderOld(Integer choice, String owner, String repo) {
		this.owner = owner;
		this.repo = repo;
		interval = choice;
		computeTimeStamps();
	...
	private void computeTimeStamps() {
		...
		int i = 0;
		for (i = interval; i <= months; i+=interval) {
			timeList.add(addMonths(startDate,i));
		}
		if (i>months){
			timeList.add(now);
		}
